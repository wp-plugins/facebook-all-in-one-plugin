...
class wdfbaio_Installer {
	...
	*/
	...
	function wdfbaio_Installer () {
		$this->__construct();
	}

	function check () {
		$me = new wdfbaio_Installer;
		$me->install();
	}

	function install () {
		$sections = array_keys($this->_defaults);
		$api = get_site_option('wdfbaio_api');
		$propagate_api = isset($api['allow_propagation']) ? (int)$api['allow_propagation'] : false;

		foreach ($sections as $section) {
			$opts = get_option('wdfbaio_' . $section, false);
			$site_opts = get_site_option('wdfbaio_' . $section, false);
			if (!$site_opts || !is_array($site_opts)) {
				//update_site_option('wdfbaio_' . $section, $this->_defaults[$section]);
			...
			if ('api' == $section && $propagate_api) {
				...
				update_option('wdfbaio_' . $section, $res);
				$this->data->set_key('wdfbaio_' . $section, $res);
			} else {
				$opts = is_array($opts) ? $opts : array();
				$this->data->set_key('wdfbaio_' . $section, $opts);
			}
		}
	}
