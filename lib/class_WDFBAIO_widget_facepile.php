 ...
 */
class wdfbaio_WidgetFacepile extends WP_Widget {

	function wdfbaio_WidgetFacepile () {
		$widget_ops = array('classname' => __CLASS__, 'description' => __('Shows Facebook Facepile box', 'wdfbaio'));
	...
	function form($instance) {
		...
		$html .= '<p>';
		$html .= '<label for="' . $this->get_field_id('url') . '">' . __('URL <small>(optional)</small>:', 'wdfbaio') . '</label>';
		$html .= '<input type="text" name="' . $this->get_field_name('url') . '" id="' . $this->get_field_id('url') . '" class="widefat" value="' . $url . '"/>';
		$html .= '<div>' . __('By default, the widget will show the Facepile box for your page. You can override that by providing a custom URL here.', 'wdfbaio') . '</div>';
		$html .= '</p>';
		...
		$html .= '<p>';
		$html .= '<label for="' . $this->get_field_id('rows') . '">' . __('Rows:', 'wdfbaio') . '</label>';
