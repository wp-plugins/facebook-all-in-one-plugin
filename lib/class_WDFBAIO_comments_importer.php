<?php
class wdfbaio_CommentsImporter {
	...
	function wdfbaio_CommentsImporter () { $this->__construct(); }
	...
	function process_comments ($post_id, $item_id) {
		...
		foreach ($comments as $comment) {
			...
			add_comment_meta($comment_id, 'wdfbaio_comment', $meta) ;

			if ($this->model->data->get_option('wdfbaio_comments', 'notify_authors')) {
	...
	function import_comments () {
		$limit = (int)$this->model->data->get_option('wdfbaio_comments', 'comment_limit');
		$limit = $limit ? $limit : 10;

		$tokens = $this->model->data->get_option('wdfbaio_api', 'auth_tokens');
		$skips = $this->model->data->get_option('wdfbaio_comments', 'skip_import');
	 ...
	 */
	function serve () {
		$me = new wdfbaio_CommentsImporter;
		$me->import_comments();
	}
}